...
function App() {
  ...
  const isPromptCard = (card: Card) =>
    card.tag.replace(/\s+/g, "").toUpperCase().startsWith("PROMPT");
  const [isCute, setIsCute] = useState(true);
  const isCuteCollapsed = false;
  const [cards, setCards] = useState<Card[]>(() =>
    normalizeCardCategories(loadPersistedCards(CARDS_STORAGE_KEY, CARDS)),
  );
  const [slotCardIdBySlot, setSlotCardIdBySlot] = useState<
    Record<number, string | null>
  >({});
  const [lastCardIdBySlot, setLastCardIdBySlot] = useState<
  ...
  const [slotCooldownUntil, setSlotCooldownUntil] = useState<
    Record<number, number>
  >(() => loadPersistedCooldowns());
  const [isSheetOpen, setIsSheetOpen] = useState(false);
  const [newCardCategory, setNewCardCategory] = useState<"standard" | "prompt">(
    "standard",
  );
  ...
  const [radioSessionByStation, setRadioSessionByStation] =
    useState<PersistedRadioSessionByStation>(() =>
      loadPersistedRadioSessionState(),
    );
  const [uploadedTunesByStation, setUploadedTunesByStation] =
    useState<UploadedTunesByStation>({
      hype: [],
      night: [],
    });
  const [tunerPosition, setTunerPosition] = useState(
    RADIO_STATIONS[0].position,
  );
  ...
  const activeStationIdRef = useRef<RadioStation["id"] | null>(
    RADIO_STATIONS[0].id,
  );
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        SLOT_COOLDOWN_STORAGE_KEY,
        JSON.stringify(slotCooldownUntil),
      );
    } catch {
  ...
  useEffect(() => {
    setSlotCooldownUntil((prev) => {
      ...
      const next = Object.entries(prev).reduce<Record<number, number>>(
        (acc, [key, value]) => {
          if (value > nowMs) {
  ...
  useEffect(() => {
    if (newCardCategory === "prompt") {
      setNewTag((current) =>
        ...
        !current.replace(/\s+/g, "").toUpperCase().startsWith("PROMPT")
          ? "PROMPT"
          : current,
      );
      setNewTitle((current) =>
        current === "REACTION TEST" ? "TINY REFLECTION" : current,
      );
      setNewBody((current) =>
    ...
    setNewTag((current) =>
      current.trim().length === 0 ||
      current.replace(/\s+/g, "").toUpperCase() === "PROMPT"
        ? "BODY\nRESET"
        : current,
    );
    setNewTitle((current) =>
      current === "TINY REFLECTION" ? "REACTION TEST" : current,
    );
    setNewBody((current) =>
  ...
  useEffect(() => {
    ...
    try {
      window.sessionStorage.setItem(
        RADIO_SESSION_STORAGE_KEY,
        JSON.stringify(radioSessionByStation),
      );
    } catch {
      // Ignore storage failures (private mode / quota).
    }
  ...
  const monthYearLabel = now.toLocaleDateString([], {
    month: "2-digit",
    day: "2-digit",
    year: "numeric",
  });
  ...
  const activeStationUploadedTunes = activeStation
    ? (uploadedTunesByStation[activeStation.id] ?? [])
    : [];
  const dayOfWeek = now.getDay();
  const hourFloat = now.getHours() + now.getMinutes() / 60;
  const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
  const clampPercent = (value: number) =>
    Math.round(Math.max(5, Math.min(100, value)));
  ...
  useEffect(() => {
    ...
    const parseTracks = (
      ...
      tracks
        ...
        .map((track) => ({
          ...track,
          file: track.file.includes("/")
            ? track.file
            : `${stationId}/${track.file}`,
        }));

    const loadUploadedTunes = async () => {
      try {
        const response = await fetch("/hype-fm/library2.json", {
          cache: "no-store",
        });
        if (!response.ok) {
          return;
        ...
        const json = (await response.json()) as {
          tracks?: Array<{ title?: string; file?: string }>;
          stations?: Partial<
            Record<RadioStation["id"], Array<{ title?: string; file?: string }>>
          >;
        };
        if (!mounted) {
          return;
  ...
  const scheduleRadioStep = () => {
    ...
    const station =
      stationById(activeStationIdRef.current) ?? RADIO_STATIONS[0];
    ...
    if (nextNoteIndex >= song.notes.length) {
      ...
      const nextPoolIndex =
        currentPoolIndex >= 0 ? (currentPoolIndex + 1) % songPool.length : 0;
  ...
  const playUploadedTrack = async (
    ...
    audio.onloadedmetadata = async () => {
      if (
        Number.isFinite(startAtSec) &&
        Number.isFinite(audio.duration) &&
        audio.duration > 1
      ) {
        audio.currentTime = Math.min(
          Math.max(startAtSec ?? 0, 0),
    ...
    audio.onended = () => {
      ...
      const nextTrackIndex =
        (uploadedIndexRef.current + 1) % stationTunes.length;
      uploadedIndexRef.current = nextTrackIndex;
      setRadioSessionByStation((prev) => ({
        ...prev,
        [stationId]: {
          mode: "uploads",
          uploadedTrackIndex: nextTrackIndex,
          uploadedTimeSec: 0,
        },
      }));
  ...
  const startStationPlayback = (station: RadioStation) => {
    ...
    tuneTimerRef.current = window.setTimeout(
      () => {
        ...
        setIsTuning(false);
        if (useUploads) {
          void playUploadedTrack(
            station.id,
            uploadedIndexRef.current,
            restoredUploadTimeSec === undefined,
            restoredUploadTimeSec,
          );
        } else {
  ...
  useEffect(() => {
    ...
    if (
      currentPlaybackStationIdRef.current &&
      currentPlaybackStationIdRef.current !== activeStationId
    ) {
      persistStationPlaybackState(currentPlaybackStationIdRef.current);
    }
    if (!activeStation) {
  ...
  useEffect(() => {
    ...
    setSlotCardIdBySlot((prev) => {
      ...
      const pickRandomCard = (pool: Card[]) =>
      ...
      for (let slotIndex = 0; slotIndex < CARD_SLOT_COUNT; slotIndex += 1) {
        ...
        if (
          currentCardId &&
          currentCardsById.has(currentCardId) &&
          !usedIds.has(currentCardId)
        ) {
        ...
        const preferredPool = availableCards.filter(
          (card) => card.id !== previousCardId,
        );
        const preferred = pickRandomCard(
          preferredPool.length > 0 ? preferredPool : availableCards,
        );
  ...
  const handleVote = (cardId: string, direction: -1 | 1) => {
    ...
    const slotIndex = visibleSlots.findIndex(
      (slot) => slot.card?.id === cardId,
    );
  ...
  const addCard = () => {
    ...
    setCards((prev) => [
      ...prev,
      { id, tag, title, body, paws: PAW_SETS.greenRed },
    ]);
    if (newCardCategory === "prompt") {
      setNewTag("PROMPT");
  ...
  return (
    <main className={`app ${isCute ? "app--cute" : "app--stealth"}`}>
                    ...
                    <span className="cute-status-icons">
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            setIsBatteryOverlayOpen((open) => !open)
                          }
                        >
                          <span className="cute-battery" />
                        </button>
                        <div
                          className={`cute-battery-popup ${isBatteryOverlayOpen ? "is-open" : ""}`.trim()}
                          role="status"
                          aria-live="polite"
                        >
                          <span
                            className="cute-battery-popup__arrow"
                            aria-hidden="true"
                          />
                          <p>BATTERY VALUES</p>
                          <div className="cute-battery-popup__group">
                            <div className="cute-battery-popup__item">
                              ...
                              <span className="cute-battery-popup__line">
                                <span
                                  className="cute-battery-popup__fill cute-battery-popup__fill--social"
                                  style={{ width: meterWidth(socialBattery) }}
                                />
                              </span>
                            </div>
                            <div className="cute-battery-popup__item">
                              ...
                              <span className="cute-battery-popup__line">
                                <span
                                  className="cute-battery-popup__fill cute-battery-popup__fill--sleep"
                                  style={{ width: meterWidth(sleepBattery) }}
                                />
                              </span>
                            </div>
                            <div className="cute-battery-popup__item">
                              ...
                              <span className="cute-battery-popup__line">
                                <span
                                  className="cute-battery-popup__fill cute-battery-popup__fill--attention"
                                  style={{
                                    width: meterWidth(attentionBattery),
                                  }}
                                />
                              </span>
                            </div>
                  ...
                  {isAllCardsPawSorted ? (
                    ...
                    visibleSlots.map((slot, index) => {
                      ...
                      if (!card) {
                        const minutesLeft =
                          slot.cooldownUntil === null
                            ...
                            : Math.max(
                                1,
                                Math.ceil((slot.cooldownUntil - nowMs) / 60000),
                              );
                        return (
                          <article
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`cute-card-reveal ${removingCardId === card.id ? "cute-card-reveal--visible" : ""}`.trim()}
                          >
                            {removalDirection === 1
                              ? "PAW UP LOGGED"
                              : "PAW DOWN LOGGED"}
                          </div>
                          <div className="cute-card-surface">
                  ...
                  <footer className="cute-bottom-strip">
                    <div className="cute-radio" aria-live="polite">
                      ...
                      <div className="cute-radio__lower">
                        <div>
                          <p className="cute-radio__brand">
                            {activeStation?.name ?? "NO SIGNAL"}
                          </p>
                        ...
                        <button
                          ...
                          aria-label={
                            isRadioOn ? "Turn radio off" : "Turn radio on"
                          }
                          onClick={() => void toggleRadio()}
                        >
                          <span
                            className={`cute-radio__play-icon ${isRadioOn ? "cute-radio__play-icon--pause" : "cute-radio__play-icon--play"}`}
                          >
                            {isRadioOn ? "||" : "▶"}
                          </span>
                        </button>
        ...
        <section className="stealth-shell" aria-label="Stealth interface">
          <section className="stealth-portal">
                ...
                <article className="stealth-battery-card">
                  ...
                  <div className="stealth-battery-row">
                    <span className="stealth-battery-meter" aria-hidden="true">
                      <span
                        className="stealth-battery-fill"
                        style={{ width: meterWidth(socialBattery) }}
                      />
                    </span>
                    <strong className="stealth-battery-value">
                      {socialBattery}%
                    </strong>
                  </div>
                </article>
                ...
                <article className="stealth-battery-card">
                  ...
                  <div className="stealth-battery-row">
                    <span className="stealth-battery-meter" aria-hidden="true">
                      <span
                        className="stealth-battery-fill"
                        style={{ width: meterWidth(attentionBattery) }}
                      />
                    </span>
                    <strong className="stealth-battery-value">
                      {attentionBattery}%
            ...
            <section className="stealth-results" aria-label="Stealth cards">
              {visibleSlots.map((slot, index) => {
                ...
                if (!card) {
                  ...
                  return (
                    <article
                      key={`stealth-empty-slot-${index}`}
                      className="stealth-row stealth-row--empty"
                      aria-live="polite"
                    >
            ...
            <section className="stealth-radio" aria-live="polite">
              <p className="stealth-radio__brand">
                {activeStation?.name ?? "NO SIGNAL"}
              </p>
      ...
      >
        <span
          className={`mode-label ${!isCute ? "mode-label--active" : ""}`.trim()}
        >
          STEALTH
        </span>
        <span
          className={`mode-toggle ${isCute ? "mode-toggle--right" : "mode-toggle--left"}`}
          aria-hidden
        >
                    ...
                    <tr key={card.id}>
                      <td>
                        <input
                          value={card.tag}
                          onChange={(e) =>
                            patchCard(card.id, { tag: e.target.value })
                          }
                        />
                      </td>
                      <td>
                      ...
                      <td>
                        <input
                          value={card.body}
                          onChange={(e) =>
                            patchCard(card.id, { body: e.target.value })
                          }
                        />
                      </td>
                      <td>
            ...
            <footer className="sheet-footer">
              ...
              <div className="sheet-add-row">
                <select
                  ...
                  onChange={(e) =>
                    setNewCardCategory(e.target.value as "standard" | "prompt")
                  }
                >
